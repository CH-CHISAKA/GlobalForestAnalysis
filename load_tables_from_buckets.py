...
# CONFIG
# =========================================================
# Use os.getenv to pull the keys safely from the environment
...
# The client remains the same, but now uses the safe variables
...
def upsert_rows(table_name: str, rows: List[Dict[str, Any]], on_conflict: str, batch_size: int = 500) -> None:
    ...
    for batch in chunked(rows, batch_size):
        ...
        # =========================================================
        # NORMALIZERS
        # Adjust only if your JSON field names differ
        # =========================================================
...
{
    "iso": r["iso"],
    "latest_actual_annual_loss_ha": r.get("latest_actual_annual_loss_ha"),
    "avg_forecast_annual_loss_ha": r.get("avg_forecast_annual_loss_ha"),
    "max_forecast_annual_loss_ha": r.get("max_forecast_annual_loss_ha"),
    ...
    return [
{
    ...
    "annual_loss_ha": r.get("annual_loss_ha"),
    "annual_primary_loss_ha": r.get("annual_primary_loss_ha"),
    "countries_count": r.get("countries_count"),
}
for r in rows
]
