...
2. Create and activate a **virtual environment** to keep project dependencies isolated from the system Python packages.
  ...
  **Part A**
  ...
  - Importance of using a virtual environment
    ...
    - Reproducibility 
      - You can "freeze" your environment into a `requirement.txt` file. Others (or your future self) can recreate the same environment later.
    - Cleaner System
    ...
    - To activate the virtual environment, use the following commands:
      - For Windows (via Git Bash) - **Git Bash is the preferred option**:
    ...
    - Confirm that the virtual environment is active by executing the following     (**use Git Bash** if you are running it on Windows OS). It should show the
